...
if [ ${STATUS} != "stopped" ]; then
  ...
  echo "Please:"
  echo ""
  echo "1. Remember to export the PROJECT_DIR and minishift paths"
  echo ""
  echo "export PROJECT_DIR=</path/of/project>"
  echo "export PATH=$PATH:/home/ec2-user/minishift-1.34.2-linux-amd64/"
  echo ""
  echo "2. To set oc cli running the following command:"
  echo ""
  echo "minishift oc-env"
  echo ""
  echo "3. Export path as the command suggests"
  echo ""
  echo "4. Run the following command to get info about user and project"
  echo ""
  echo "oc status"
  echo ""
  echo "---------------------------------------------------------------"
